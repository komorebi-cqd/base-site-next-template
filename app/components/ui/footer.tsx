...
const Footer = async ({ locale }: { locale: string }) => {
  ...
  const productList = [
    ...
    {
      id: 3,
      text: intl.formatMessage({ id: "nav_three" }),
  ...
  return (
    <footer className="w-full pb-12 md:pb-[13.375rem] bg-[#071532] pt-[82px] px-10 md:px-0">
      <div className="w-1200 text-white flex flex-col md:flex-row justify-between items-start gap-y-6">
        ...
        {/* 右边产品和公司信息 */}
        <div
          className={`flex flex-col md:flex-row text-[#96BAFF] gap-y-6 md:gap-y-0  ${
            locale === "en" ? " gap-x-10" : "md:gap-x-44"
          }`}
        >
          {/* 产品 */}
          <div>
            <div className="text-white text-base font-semibold mb-5">
              {intl.formatMessage({ id: "foot_t" })}
            </div>
            <ul className="flex flex-col gap-y-2 text-sm">
              {productList.map((product) => (
                <li key={product.id} className="">
                  <Link
                    href={locale === "en" ? "/en" + product.link : product.link}
                  >
                    {product.text}
                  </Link>
                </li>
          ...
          {/* 公司信息 */}
          <div>
            <div className="text-white text-base font-semibold mb-5 whitespace-nowrap">
              {intl.formatMessage({ id: "foot_company" })}
            </div>
            <ul className=" mb-6 text-sm">
              <li>
                <Link href={locale === "en" ? "/en/about" : "/about"}>
                  {intl.formatMessage({ id: "nav_six" })}
                </Link>
              </li>
            </ul>
            <Image
              src="/image/footer/foot-ewm.png"
              width={140}
              height={140}
              alt="WeTech-phone"
            />
