...
const PageTop: React.FC<Props> = ({ bgImg, title, desc, topText }) => {
    ...
    return (
        <div className={`w-full h-[51.125rem] bg-center bg-no-repeat relative bg-cover mb-[14.5rem]`} style={{ backgroundImage: `url(${bgImg})` }}>
            ...
            <div className='flex flex-col justify-between items-center absolute -bottom-52 left-1/2 -translate-x-1/2 w-[77.25rem] h-[20.25rem] bg-gradient-to-b from-[#EEF6FF] to-white rounded-[1.69rem] shadow-[0_12px_3px_0px_rgba(49,111,231,0.1)]'>
                {/* 标题 */}
                <div className='flex items-center justify-center gap-x-5 pt-12 mb-5'>
                ...
                {/* 介绍 */}
                <div className="text-[#333] text-base text-center flex-1">
                    {desc}
                </div>
                {/* 按钮 */}
